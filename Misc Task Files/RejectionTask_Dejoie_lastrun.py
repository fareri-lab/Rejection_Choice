...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/jordansiegel/Documents/PhD/Research/Second Year Project/Rejection_Task 2/RejectionTask_Dejoie_lastrun.py',
...
# Initialize components for Routine "Block1_Continue"
Block1_ContinueClock = core.Clock()
...
nerdface = visual.ImageStim(
    win=win,
    name='nerdface', 
    image='Images/nerd emoji.jpeg', mask=None, anchor='center',
...
block2_lowerchoice = visual.TextStim(win=win, name='block2_lowerchoice',
    text='Lower',
    font='Open Sans',
    pos=(-.3, -.25), height=0.1, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
    depth=-2.0);
...
image_8 = visual.ImageStim(
    win=win,
    name='image_8', 
    image='Images/sillyemoji.webp', mask=None, anchor='center',
    ori=0.0, pos=(0, 0), size=(0.5, 0.5),
    color=[1,1,1], colorSpace='rgb', opacity=None,
    flipHoriz=False, flipVert=False,
...
block2_totalloop = data.TrialHandler(nReps=1.0, method='random', 
...
for thisBlock2_totalloop in block2_totalloop:
    ...
    # ------Prepare to start Routine "Block1_Continue"-------
    continueRoutine = True
    # update component parameters for each repeat
    key_resp_15.keys = []
    key_resp_15.rt = []
    ...
    # keep track of which components have finished
    Block1_ContinueComponents = [block1_continue, key_resp_15]
    for thisComponent in Block1_ContinueComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    t = 0
    _timeToFirstFrame = win.getFutureFlipTime(clock="now")
    Block1_ContinueClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
    frameN = -1
    
    # -------Run Routine "Block1_Continue"-------
    while continueRoutine:
        # get current time
        t = Block1_ContinueClock.getTime()
        tThisFlip = win.getFutureFlipTime(clock=Block1_ContinueClock)
        tThisFlipGlobal = win.getFutureFlipTime(clock=None)
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
    ...
    # -------Ending Routine "Block1_Continue"-------
    for thisComponent in Block1_ContinueComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    block2_totalloop.addData('block1_continue.started', block1_continue.tStartRefresh)
    block2_totalloop.addData('block1_continue.stopped', block1_continue.tStopRefresh)
    # check responses
    if key_resp_15.keys in ['', [], None]:  # No response was made
        key_resp_15.keys = None
    block2_totalloop.addData('key_resp_15.keys',key_resp_15.keys)
    if key_resp_15.keys != None:  # we had a response
        block2_totalloop.addData('key_resp_15.rt', key_resp_15.rt)
    block2_totalloop.addData('key_resp_15.started', key_resp_15.tStartRefresh)
    block2_totalloop.addData('key_resp_15.stopped', key_resp_15.tStopRefresh)
    # the Routine "Block1_Continue" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
    
# completed 1.0 repeats of 'block2_totalloop'
...
for thisBlock2_entireloop in block2_entireloop:
    ...
    for thisBlock2_photoloop in block2_photoloop:
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            if text_14.status == STARTED:  # only update if drawing
                text_14.setText(Response, log=False)
            ...
            if image_11.status == STARTED:  # only update if drawing
                image_11.setImage(Graphic, log=False)
    ...
    # keep track of which components have finished
    Block2_LotteryselfchoiceComponents = [instructionsforselfchoice2, block2_higherchoice, block2_lowerchoice, key_resp_8, block2_responsefeedback]
    ...
    for thisBlock3_photoloop in block3_photoloop:
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            if text_11.status == STARTED:  # only update if drawing
                text_11.setText(Response, log=False)
            ...
            if image_10.status == STARTED:  # only update if drawing
                image_10.setImage(Graphic, log=False)
