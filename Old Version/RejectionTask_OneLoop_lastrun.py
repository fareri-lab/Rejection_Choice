...
ConditionselectionClock = core.Clock()
if condition[block] == 'Rej':
    condition_outcome_freq = 70
    partner = 'Charlie'       
    feedbackresponses = '%s %s your photo.'%(partner,'disliked')
    print(condition[block])
    print(condition_outcome_freq)
    print(partner)
    print(feedbackresponses)
elif condition[block] == 'Acc':
    condition_outcome_freq = 30
    partner = 'Sam'
    feedbackresponses ='%s %s your photo.'%(partner,'disliked') 
    print(condition[block])
    print(condition_outcome_freq)
    print(partner)
    print(feedbackresponses) 
elif condition[block] == 'Neutral':
    condition_outcome_freq = 50
    partner = 'Riley'
    feedbackresponses ='%s %s your photo.'%(partner,'disliked')
    print(condition[block])
    print(condition_outcome_freq)
...
Youhavematched = visual.TextStim(win=win, name='Youhavematched',
    ...
    languageStyle='LTR',
    depth=0.0);
PartnerName = visual.TextStim(win=win, name='PartnerName',
    text=partner,
    font='Open Sans',
...
partnername = visual.TextStim(win=win, name='partnername',
    text=partner,
    font='Open Sans',
    pos=(.2, 0.45), height=0.05, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
    depth=0.0);
Photo_One = visual.TextStim(win=win, name='Photo_One',
    text='This photo is now being shared with',
    font='Open Sans',
    pos=(-0.08, .45), height=0.05, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
    depth=-1.0);
Image_One = visual.ImageStim(
    win=win,
    name='Image_One', 
    image='Images/Image_One.jpeg', mask=None, anchor='center',
...
entiretaskloop = data.TrialHandler(nReps=5.0, method='random', 
...
for thisEntiretaskloop in entiretaskloop:
    ...
    for thisComponent in ConditionselectionComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    t = 0
    _timeToFirstFrame = win.getFutureFlipTime(clock="now")
    ConditionselectionClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
    frameN = -1
    ...
    while continueRoutine:
        ...
        tThisFlip = win.getFutureFlipTime(clock=ConditionselectionClock)
        tThisFlipGlobal = win.getFutureFlipTime(clock=None)
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
        block = block+1
    ...
    _key_resp_3_allKeys = []
    # keep track of which components have finished
    Partner_MatchComponents = [Youhavematched, PartnerName, image, key_resp_3, PressToContinue]
    ...
    while continueRoutine:
        ...
        # *PartnerName* updates
        if PartnerName.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            PartnerName.frameNStart = frameN  # exact frame index
            PartnerName.tStart = t  # local t and not account for scr refresh
            PartnerName.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(PartnerName, 'tStartRefresh')  # time at next scr refresh
            PartnerName.setAutoDraw(True)
    ...
    for thisComponent in Partner_MatchComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    entiretaskloop.addData('Youhavematched.started', Youhavematched.tStartRefresh)
    entiretaskloop.addData('Youhavematched.stopped', Youhavematched.tStopRefresh)
    entiretaskloop.addData('PartnerName.started', PartnerName.tStartRefresh)
    entiretaskloop.addData('PartnerName.stopped', PartnerName.tStopRefresh)
    ...
    for thisChoiceloop in choiceloop:
        ...
        for thisPhotoloop in photoloop:
            ...
            # update component parameters for each repeat
            # keep track of which components have finished
            Photo_ShareComponents = [partnername, Photo_One, Image_One, text_8]
            ...
            while continueRoutine and routineTimer.getTime() > 0:
                ...
                if partnername.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    ...
                    win.timeOnFlip(partnername, 'tStartRefresh')  # time at next scr refresh
                    partnername.setAutoDraw(True)
                if partnername.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > partnername.tStartRefresh + 3-frameTolerance:
                        # keep track of stop time/frame for later
                        partnername.tStop = t  # not accounting for scr refresh
                        partnername.frameNStop = frameN  # exact frame index
                        win.timeOnFlip(partnername, 'tStopRefresh')  # time at next scr refresh
                        partnername.setAutoDraw(False)
            ...
            for thisComponent in Photo_ShareComponents:
                if hasattr(thisComponent, "setAutoDraw"):
                    thisComponent.setAutoDraw(False)
            photoloop.addData('partnername.started', partnername.tStartRefresh)
            photoloop.addData('partnername.stopped', partnername.tStopRefresh)
            ...
            while continueRoutine and routineTimer.getTime() > 0:
                ...
                if text_4.status == STARTED:  # only update if drawing
                    text_4.setText(feedbackresponses, log=False)
...
# completed 5.0 repeats of 'entiretaskloop'
